...
# from helpers.get_email_remix_format import get_email_remix_format
...
def get_email_remix_format():
    return {
        'subject': '<subject text limited to 15 words>',
    ...
    try:
        ...
        response = await asyncio.wait_for(
            asyncio.to_thread(
                ...
                messages=[
                    {"role": "system",
                     "content": get_email_remix_system_prompt(get_email_remix_format())},
                    {"role": "user", "content": get_email_remix_user_prompt(
                        news_content_list, get_email_remix_format())}
                ],
                temperature=0.9,
            ),
            timeout=90
        ...
        print("json ", json.loads(parsed_content))
